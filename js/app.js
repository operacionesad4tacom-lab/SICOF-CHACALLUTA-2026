...
async function cargarDashboard() {
    try {
        ...
        // LISTA DE SERVICIOS ÚLTIMAS 24H
        const container = document.getElementById('listaServicios24h');
        
        if (ultimas24h.length === 0) {
            container.innerHTML = '<div class="alert alert-info">No hay servicios registrados en las últimas 24 horas</div>';
        } else {
            ...
            html += `
                <div class="ranking-row header">
                    ...
                    <div style="text-align: center;">Infracciones</div>
                    <div style="text-align: center;">Detenidos</div>
                    <div style="text-align: center;">Hora</div>
                </div>
            `;
            
            // Ordenar por más reciente
            ultimas24h.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
            
            ultimas24h.forEach(s => {
                const cuartelInfo = window.SICOF_CONFIG.cuarteles.find(c => c.codigo === s.cuartel_codigo);
                const nombreCuartel = cuartelInfo ? cuartelInfo.nombre : s.cuartel_codigo;
                ...
                const horaRegistro = new Date(s.created_at).toLocaleTimeString('es-CL', { 
                    hour: '2-digit', 
                    minute: '2-digit' 
                });
                
                html += `
                    <div class="ranking-row" style="cursor: pointer;" onclick="verDetalleServicio('${s.id}')">
                        <div><strong>${nombreCuartel}</strong></div>
                        <div>${s.nombre_servicio}</div>
                        <div style="text-align: center;">
                            <span class="badge badge-success">${formatNumber(totalControles)}</span>
                        </div>
                        <div style="text-align: center;">
                            <span class="badge ${totalInfracciones > 0 ? 'badge-warning' : 'badge-success'}">${formatNumber(totalInfracciones)}</span>
                        </div>
                        <div style="text-align: center;">
                            <span class="badge ${s.detenidos_cantidad > 0 ? 'badge-danger' : 'badge-success'}">${formatNumber(s.detenidos_cantidad || 0)}</span>
                        </div>
                        <div style="text-align: center; font-size: 0.875rem; color: var(--gris-medio);">
                            ${horaRegistro}
                        </div>
                    </div>
                `;
...
function handlePaso3Submit(e) {
    ...
    // Guardar datos (sin validaciones de límite superior)
...
// ========== RANKING ==========
let rankingCuartelSeleccionado = null;
...
async function cargarRanking() {
    ...
    try {
        ...
        if (rankingCuartelSeleccionado) {
            // Mostrar ranking de servicios del cuartel
            mostrarRankingServicios(data, rankingCuartelSeleccionado);
        } else {
            // Mostrar ranking general de cuarteles
            mostrarRankingCuarteles(data);
        }
...
function mostrarRankingCuarteles(servicios) {
    // Calcular ranking por cuartel
    const porCuartel = {};
    servicios.forEach(s => {
        if (!porCuartel[s.cuartel_codigo]) {
            porCuartel[s.cuartel_codigo] = {
        ...
        if (s.hitos_planificados > 0 || s.pnh_planificados > 0 || s.sitios_planificados > 0) {
            const planTotal = s.hitos_planificados + s.pnh_planificados + s.sitios_planificados;
            const realTotal = s.hitos_realizados + s.pnh_realizados + s.sitios_realizados;
            const cum = planTotal > 0 ? (realTotal / planTotal) * 100 : 0;
    ...
    // Actualizar UI
    document.getElementById('rankingTitle').textContent = '🏆 Ranking de Cuarteles';
    document.getElementById('rankingSubtitle').textContent = 'Clasificación por desempeño operacional - Click para ver servicios';
    document.getElementById('rankingBreadcrumb').style.display = 'none';
    ...
    ranking.forEach((c, index) => {
        ...
        html += `
            <div class="ranking-row" style="cursor: pointer;" onclick="verRankingCuartel('${c.codigo}')">
...
function mostrarRankingServicios(todosServicios, codigoCuartel) {
    // Filtrar solo servicios de este cuartel
    const servicios = todosServicios.filter(s => s.cuartel_codigo === codigoCuartel);
    
    // Calcular puntuación por servicio
    const ranking = servicios.map(s => {
        ...
        const planTotal = s.hitos_planificados + s.pnh_planificados + s.sitios_planificados;
        const realTotal = s.hitos_realizados + s.pnh_realizados + s.sitios_realizados;
        const cumplimiento = planTotal > 0 ? (realTotal / planTotal) * 100 : 0;
        
        const puntuacion = (totalControles * 2) + cumplimiento + (s.detenidos_cantidad * 5);
    ...
    // Actualizar UI
    const cuartelInfo = window.SICOF_CONFIG.cuarteles.find(c => c.codigo === codigoCuartel);
    document.getElementById('rankingTitle').textContent = `🏆 Ranking de Servicios`;
    document.getElementById('rankingSubtitle').textContent = cuartelInfo ? cuartelInfo.nombre : codigoCuartel;
    document.getElementById('rankingBreadcrumb').style.display = 'block';
    ...
    ranking.forEach((s, index) => {
        const positionClass = index < 3 ? `position-${index + 1}` : '';
        const scorePercent = (s.puntuacion / maxPuntuacion) * 100;
        ...
        html += `
            <div class="ranking-row">
                ...
                <div>
                    ...
                    <div style="font-size: 0.875rem; color: var(--gris-medio);">
                ...
                <div style="text-align: center;">${formatFecha(s.fecha)}</div>
                <div style="text-align: center;">
                    <span class="badge badge-success">${formatNumber(s.totalControles)}</span>
                </div>
                <div style="text-align: center;">
                    <span class="badge ${getBadgeByPercentage(s.cumplimiento)}">
                        ${formatPorcentaje(s.cumplimiento)}
                    </span>
                </div>
                <div style="text-align: center; font-weight: bold; color: var(--verde-oficial);">
...
function verRankingCuartel(codigoCuartel) {
    rankingCuartelSeleccionado = codigoCuartel;
    cargarRanking();
}

function volverRankingGeneral() {
    rankingCuartelSeleccionado = null;
    cargarRanking();
}
...
function verDetalleServicio(servicioId) {
    // Por ahora muestra toast, puede expandirse a modal con detalles completos
    showToast('Click en servicio registrado - Funcionalidad en desarrollo', 'info');
}
